...
class SalienceEngine:
    ...
    def _compute_thread_membership(self, concept) -> float:
        ...
        try:
            # DEBT-242: retrieval must not import features (Contract 5).
            compute_thread_membership = __import__("importlib").import_module("app.features.threads").compute_thread_membership
...
def recompute_salience(
    ...
    if active_goals is None:
        try:
            from app.storage import read_snapshot_db, get_related_concepts

            with read_snapshot_db("salience_goals") as conn:  # T2-3: RLock-free
    ...
    try:
        preload_thread_membership_cache = __import__("importlib").import_module("app.features.threads").preload_thread_membership_cache  # DEBT-242
