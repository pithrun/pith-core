...
def walk_to_chain_head(
    concept_id: str,
    conn,
    max_depth: int = 8,
) -> tuple[str | None, int]:
    """Walk superseded_by chain to find the current head concept.

    RETRIEVAL-101: Reusable chain-walk for both MCP endpoint and retrieval pipeline.

    Args:
        concept_id: Starting concept (may be stale).
        conn: SQLite connection (caller provides — no global state).
        max_depth: Safety limit to prevent infinite loops.

    Returns:
        (head_id, chain_length) where head_id is the current-head concept ID,
        or (None, 0) if concept has no supersession chain.
        If head_id == concept_id, the concept IS the current head.
    """
    current = concept_id
    visited = {concept_id}
    depth = 0
    ...
    while depth < max_depth:
        ...
        if not row or not row[0]:
            break
        successor = row[0]
        if _is_sentinel(successor) or successor in visited:
            break

        # Verify successor exists (dangling reference protection)
        exists = conn.execute(
            "SELECT 1 FROM concepts WHERE id = ?", (successor,)
        ).fetchone()
        if not exists:
            break
...
def pith_evolution_of(concept_id: str) -> dict:
    ...
    Returns {"status": "success", "chain": [...], "current_version": "..."}.
    """
    try:
        conn = _get_connection()
        ...
        # RETRIEVAL-101: Reuse walk_to_chain_head for the forward walk
        head_id, depth = walk_to_chain_head(concept_id, conn, max_depth=20)

        if head_id is None:
            return {
                "status": "success",
                "chain": [concept_id],
                "current_version": concept_id,
            }

        # Rebuild full chain for the response
        chain = [concept_id]
        current = concept_id
        visited = {concept_id}
        while current != head_id:
            row = conn.execute(
                "SELECT superseded_by FROM concepts WHERE id = ?", (current,)
            ).fetchone()
            if not row or not row[0] or _is_sentinel(row[0]) or row[0] in visited:
                break
            chain.append(row[0])
            visited.add(row[0])
            current = row[0]
