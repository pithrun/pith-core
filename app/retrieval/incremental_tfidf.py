...
class IncrementalTfidfIndex:
    ...
    def contains_active_concept(self, concept_id: str) -> bool:
        """Return whether concept_id points to a non-deleted index row."""
        with self.lock:
            idx = self.concept_id_to_idx.get(concept_id)
            return idx is not None and idx not in self.deleted_indices
    ...
    def save(self, index_path: str):
        ...
        with self.lock:
            ...
            # RETRIEVAL-118: Snapshot mutable index state while holding the
            # lock, then perform expensive serialization outside the lock so
            # foreground search does not block on disk I/O.
            metadata = {
            ...
            vocabulary_data = {
                ...
                "reverse_vocabulary": dict(self.reverse_vocabulary),
                "next_term_id": self.next_term_id,
            }
            document_map = {
                "concept_ids": list(self.concept_ids),
                "concept_id_to_idx": dict(self.concept_id_to_idx),
                "deleted_indices": list(self.deleted_indices),
            }
            document_frequencies = self.document_frequencies.copy()
            term_totals = self.term_totals.copy()
            tfidf_matrix = self.tfidf_matrix.copy() if self.tfidf_matrix is not None else None
            document_term_counts = [dict(term_counts) for term_counts in self.document_term_counts]
        ...
        try:
            ...
            # === NUMPY ARRAYS ===
            np.save(tmp_dir / "document_frequencies.npy", document_frequencies)
            np.save(tmp_dir / "term_totals.npy", term_totals)

            # === TF-IDF MATRIX ===
            if tfidf_matrix is not None:
                save_npz(tmp_dir / "tfidf_matrix.npz", tfidf_matrix)

            # === TERM COUNTS (compressed) ===
            term_counts_json = json.dumps(document_term_counts)
            ...
            logger.info(
                "STABILITY-020: Index saved atomically: "
                f"{metadata['document_count']} docs, {metadata['vocabulary_size']} terms"
            )
