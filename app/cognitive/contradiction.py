...
from app.core.config import CROSS_KA_DAMPENING, FEATURE_FLAGS, TIER2_AMBIGUOUS_LOW
from app.core.constants import (
    GOV_EVENT_CONTRADICTION_DETECTED,
    GOV_EVENT_CONTRADICTION_PHASE_2_COMPLETED,
    GOV_EVENT_TIER2_LLM_COMPLETED,
)
from app.core.metrics_facade import metrics as _m4
...
CROSS_TOPIC_OVERLAP_MIN = 0.15  # ~3 shared words in typical 20-word summaries (raised from 0.08 per CONTRA-017)

# CONTRA-017: Phase 1 per-call cap and hub detection
PHASE_1_MAX_PER_CALL = 5         # Max Phase 1 detections per detect_retrieval_contradictions call
PHASE_1_HUB_THRESHOLD = 3        # If one concept appears in >= this many Phase 1 pairs, downgrade to SOFT
CONTRADICTION_MODE_FULL = "full"
CONTRADICTION_MODE_LITE = "lite"
CONTRADICTION_MODE_EMERGENCY = "emergency_minimal"
...
def detect_retrieval_contradictions(
    ...
    *,
    mode: str = CONTRADICTION_MODE_FULL,
    max_survivors: int | None = None,
    persist_resolutions: bool = True,
    deadline=None,
    ...
    Args:
        ...
        skip_phase_1: If True, bypass Phase 1 keyword negation entirely.
            Set by CONTRA-018 when L1.8 handles detection at ingestion time,
            or by feature flag CONTRA_018_PHASE1_REMOVED.
        mode: full runs all enabled phases. lite caps survivors, avoids DB reads,
            embeddings, Tier 2, and persistence. emergency_minimal returns a valid
            empty result without suppressing or contesting concepts.
        max_survivors: Optional cap for lite mode.
        persist_resolutions: Whether to persist pair-level resolution outcomes.
        deadline: Optional TurnDeadline-like object for cooperative pair-loop bounds.
        max_pairs: Optional cap on actual pair checks across all detector phases.

    Returns:
        ContradictionResult with detected pairs and resolution actions.
    """
    if mode not in {CONTRADICTION_MODE_FULL, CONTRADICTION_MODE_LITE, CONTRADICTION_MODE_EMERGENCY}:
        logger.warning("Unknown contradiction detection mode %r; falling back to full", mode)
        mode = CONTRADICTION_MODE_FULL
    result = ContradictionResult()
    if mode == CONTRADICTION_MODE_EMERGENCY:
        try:
            _m4.record(
                "contradiction_detection_rate",
                0.0,
                {"pairs_evaluated": 0, "pairs_found": 0, "mode": mode},
            )
        except Exception:
            pass
        return result

    if mode == CONTRADICTION_MODE_LITE and max_survivors is not None:
        survivors = survivors[: max(2, int(max_survivors))]
    ...
    _deadline_degrade_recorded = False

    def _pair_budget_exhausted() -> bool:
        nonlocal _deadline_degrade_recorded
        if max_pairs is not None and result.pairs_checked >= max(0, int(max_pairs)):
            result.degraded = True
            result.degraded_reason = "max_pairs"
            return True
        if deadline is not None and not deadline.can_start(
            "contradiction_detection.pair_loop",
            min_remaining_ms=50.0,
        ):
            result.degraded = True
            result.degraded_reason = "deadline"
            if not _deadline_degrade_recorded:
                try:
                    deadline.skip(
                        "contradiction_detection.pair_loop",
                        "deadline_before_pair",
                        priority="required_degraded",
                        min_remaining_ms=50.0,
                    )
                except Exception:
                    pass
                _deadline_degrade_recorded = True
            return True
        return False

    def _deadline_exhausted_before_cheap_pair(phase: str) -> bool:
        nonlocal _deadline_degrade_recorded
        if deadline is None:
            return False
        if deadline.can_start(phase, min_remaining_ms=25.0):
            return False
        result.degraded = True
        result.degraded_reason = "deadline"
        if not _deadline_degrade_recorded:
    ...
    def _mark_pair_checked() -> None:
        result.pairs_checked += 1

    # Keep cheap safety passes intact; apply Stage 2 pair caps to the expensive
    # semantic phases below so flag-on mode still catches obvious contradictions.
    ...
    if mode != CONTRADICTION_MODE_LITE:
        try:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if _deadline_exhausted_before_cheap_pair("contradiction_detection.phase_05"):
                break
            if (
    ...
    # CONTRA-018: Skip Phase 1 when L1.8 handles detection at ingestion time
    _phase_1_skipped = skip_phase_1
    if not _phase_1_skipped:
        try:
            from app.core.config import get_feature_flag
            _phase_1_skipped = get_feature_flag("CONTRA_018_PHASE1_REMOVED", False)
        except Exception:
            pass  # Flag unavailable — run Phase 1 normally

    # CONTRA-017: Per-call cap + hub detection to prevent over-suppression
    t1_start = time.perf_counter()
    if _phase_1_skipped:
        logger.debug("CONTRA-018: Phase 1 skipped (skip_phase_1=%s, flag=%s)", skip_phase_1, _phase_1_skipped)
    else:
        phase_1_count = 0
        phase_1_concept_hits: dict[str, int] = {}  # concept_id -> # of Phase 1 pairs it appears in
        for i in range(n):
            for j in range(i + 1, n):
                if _deadline_exhausted_before_cheap_pair("contradiction_detection.phase_1"):
                    break
                # CONTRA-017: Hard cap — stop Phase 1 once we hit the limit
                if phase_1_count >= PHASE_1_MAX_PER_CALL:
                    break
                ...
                if pair:
                    # CONTRA-017: Hub detection — if either concept is already in too many
                    # Phase 1 pairs, downgrade from HARD to SOFT to prevent hub suppression
                    a_hits = phase_1_concept_hits.get(pair.concept_a_id, 0)
                    b_hits = phase_1_concept_hits.get(pair.concept_b_id, 0)
                    if a_hits >= PHASE_1_HUB_THRESHOLD or b_hits >= PHASE_1_HUB_THRESHOLD:
                        pair = ContradictionPair(
                            concept_a_id=pair.concept_a_id,
                            concept_b_id=pair.concept_b_id,
                            contradiction_type=ContradictionType.SOFT,
                            detection_phase=1,
                            reason=f"Negation marker asymmetry downgraded to SOFT (hub: a={a_hits}, b={b_hits})",
                        )
                    pair = _resolve_contradiction(pair, concept_map)
                    result.pairs.append(pair)
                    already_detected.add(frozenset([pair.concept_a_id, pair.concept_b_id]))
                    phase_1_count += 1
                    phase_1_concept_hits[pair.concept_a_id] = a_hits + 1
                    phase_1_concept_hits[pair.concept_b_id] = b_hits + 1
            else:
                continue
            break  # Break outer loop when inner loop breaks (cap/deadline reached)
    ...
    if mode == CONTRADICTION_MODE_LITE:
        ...
        for pair in result.pairs:
            if pair.action == ResolutionAction.SUPPRESS_LOSER:
                pair.action = ResolutionAction.PRESENT_BOTH_CONTESTED
                pair.winner_id = None
                pair.loser_id = None
                pair.reason = (pair.reason or "") + " [lite mode: suppression disabled]"
            if pair.action == ResolutionAction.PRESENT_BOTH_CONTESTED:
                result.contested_ids.extend([pair.concept_a_id, pair.concept_b_id])
            ...
            try:
                gov_ctx.log_event(
                    ...
                    details={
                        ...
                        "action": "lite_no_persistence",
                    },
                )
            except Exception as e:
                logger.warning("Failed to log lite contradiction event: %s", e)
        try:
    ...
    # --- CONTRA-018 A.4: No-embedding fallback ---
    # If Phase 1 was skipped AND no concepts have embeddings (Phase 2 dead),
    # use keyword overlap + directional opposition — Phase 2's semantic logic
    # without the cosine pre-filter. NOT Phase 1 keyword negation (99% FP).
    if _phase_1_skipped and not result.phase_2_skipped:
        _any_embeddings = any(s.embedding is not None for s in survivors)
        if not _any_embeddings and not any(p.detection_phase == 2 for p in result.pairs):
            logger.warning(
                "CONTRA-018 A.4: No embeddings available — using keyword overlap + opposition fallback"
            )
            _a4_t_start = time.perf_counter()
            _a4_count = 0
            for i in range(n):
                for j in range(i + 1, n):
                    if _pair_budget_exhausted():
                        break
                    _mark_pair_checked()
                    if _a4_count >= PHASE_1_MAX_PER_CALL:
                        break
                    if survivors[i].concept_id in always_activate_ids and survivors[j].concept_id in always_activate_ids:
                        continue
                    pair_key = frozenset([survivors[i].concept_id, survivors[j].concept_id])
                    if pair_key in already_detected:
                        continue
                    # A.4: Same logic as Phase 2 but without cosine — keyword overlap + opposition
                    if survivors[i].knowledge_area != survivors[j].knowledge_area:
                        continue
                    overlap = _compute_keyword_overlap_score(survivors[i].summary, survivors[j].summary)
                    if overlap < CROSS_TOPIC_OVERLAP_MIN:
                        continue
                    is_opposition, signal = _has_directional_opposition(survivors[i].summary, survivors[j].summary)
                    if is_opposition:
                        pair = ContradictionPair(
                            concept_a_id=survivors[i].concept_id,
                            concept_b_id=survivors[j].concept_id,
                            contradiction_type=ContradictionType.HARD,
                            detection_phase=2,
                            reason=f"A.4 no-embedding fallback: overlap={overlap:.3f}, {signal}",
                        )
                        pair = _resolve_contradiction(pair, concept_map)
                        result.pairs.append(pair)
                        already_detected.add(pair_key)
                ...
                break
            logger.debug(
                "CONTRA-018 A.4: No-embedding fallback found %d pairs in %.1fms",
                _a4_count, (time.perf_counter() - _a4_t_start) * 1000
            )
    ...
    try:
        if get_feature_flag("LLM_CONTRADICTION_TIER2_ENABLED"):
            from app.cognitive.contradiction_llm import detect_contradiction_llm, is_tier2_candidate
    ...
    if persist_resolutions and result.pairs:
        for pair in result.pairs:
...
def consume_graph_contradiction_signals(batch_size: int = 500) -> dict:
    ...
    try:
        ...
        with _db() as conn:
            # Step 1: Get selected signals and group them by normalized pair.
            # Keep event ids so cleanup deletes only events actually handled in
            # this bounded pass, not the entire backlog.
            rows = conn.execute(
                """SELECT id,
                   ...
                   LIMIT ?""",
                (batch_size * 4,),  # Over-fetch to account for duplicates
            ).fetchall()
            ...
            pair_event_ids: dict[tuple[str, str], list[int]] = {}
            for row in rows:
                event_id, src, tgt = row[0], row[1], row[2]
                if src and tgt:
                    pair = tuple(sorted([src, tgt]))
                    unique_pairs.add(pair)
                    pair_event_ids.setdefault(pair, []).append(int(event_id))
            ...
            stats["processed"] = stats["newly_resolved"] + stats["already_resolved"]

            # Step 4: Delete only selected events for handled pairs. Unselected,
            # errored, or missing-concept signals stay queued for a later pass.
            handled_event_ids: list[int] = []
            for pair in handled_pairs:
                handled_event_ids.extend(pair_event_ids.get(pair, []))
            if handled_event_ids:
                placeholders = ",".join("?" for _ in handled_event_ids)
                conn.execute(
                    f"""DELETE FROM governance_events
                        WHERE event_type = 'GRAPH_CONTRADICTION_SIGNAL'
                          AND id IN ({placeholders})""",
                    tuple(handled_event_ids),
                )
                stats["deleted_events"] = len(handled_event_ids)
                logger.info(
                    "CONTRA-002: Consumed %d selected GRAPH_CONTRADICTION_SIGNAL events, resolved %d new pairs (%d already resolved, %d errors)",
                    stats["total_events"],
