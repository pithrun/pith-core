...
from app.core.models import VerbatimFragment
...
# INGEST-041: Numbered lists (3+ consecutive items, each 10+ chars)
# Matches "1. item\n2. item\n3. item\n..." patterns
...
# INGEST-041: Bulleted lists (3+ consecutive items with - or *)
_BULLETED_LIST_RE = re.compile(
    r"((?:^[\-\*]\s+.{10,}\n?){3,})",
    re.MULTILINE,
)

# INGEST-041: Max items to store per list fragment (head + tail preservation)
_MAX_LIST_ITEMS = 50
...
def detect_verbatim_fragments(text: str) -> list[VerbatimFragment]:
    ...
    # 3.5 INGEST-041: Numbered and bulleted lists
    for list_re in (_NUMBERED_LIST_RE, _BULLETED_LIST_RE):
        for m in list_re.finditer(text):
            block = m.group(1)
            # Skip if overlaps with already-captured fragments
            if any(f[1].content and block.strip()[:50] in f[1].content for f in fragments):
                continue
            # Cap large lists: keep first 25 + last 25 items
            lines = [l for l in block.strip().split('\n') if l.strip()]
            if len(lines) > _MAX_LIST_ITEMS:
                kept = lines[:25] + [f"[... {len(lines) - 50} items omitted ...]"] + lines[-25:]
                block = '\n'.join(kept)
            _add(m.start(), block, "list")
