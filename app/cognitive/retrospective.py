...
# DEBT-229: module-level logger — replaces scattered inline `import logging` calls
logger = logging.getLogger(__name__)

from app.core.datetime_utils import _utc_now_iso
from app.cognitive.learning import create_concept, validate_proposal
...
class ConversationProcessor:
    ...
    def process_chunk(
        ...
        # INGEST-050: Capture raw chunk text as verbatim fragment for FTS retrieval.
        # Import pipeline creates concepts but historically skipped verbatim capture,
        # making imported knowledge invisible to episodic search.
        try:
            from app.cognitive.verbatim_detect import capture_conversation_verbatim
            _import_frag_ids = capture_conversation_verbatim(
                user_message=chunk,
                assistant_response="",  # Import chunks are user-sourced, no assistant
                concept_ids=[concept.id],
            )
            if _import_frag_ids:
                logger.info(
                    "INGEST-050: Import verbatim captured %d fragments for %s (chunk %d)",
                    len(_import_frag_ids), concept.id, chunk_index,
                )
        except Exception as _iv_err:
            logger.warning(
                "INGEST-050: Import verbatim capture failed (non-fatal): %s", _iv_err,
            )
        ...
        try:
            ...
            logger.warning("W8: Import quarantine failed for %s: %s", concept.id, e)
