...
def evolve_concept_nonlossy(
    ...
    try:
        ...
        if FEATURE_FLAGS.get("INGESTION_VALIDATION_ENABLED", False) and new_data.get("summary"):
            from app.policy import SUMMARY_OVERLAP_THRESHOLD, HIJACK_BLOCK_THRESHOLD, check_content_policy, compute_summary_overlap
            ...
            if old_summary and new_summary != old_summary:
                ...
                # INGEST-057: HIJACK block — refuse summary replacement for extreme rewrites
                # unless this is a type upgrade or a user correction.
                if overlap < HIJACK_BLOCK_THRESHOLD and not _has_reclassification and new_data.get("maturity") != "ESTABLISHED":
                    new_summary = old_summary  # Revert to old summary
                    merged_data["summary"] = old_summary
                    logger.warning(
                        "W9-HIJACK: Blocking summary replacement for %s "
                        "(overlap=%.2f < %.2f). Old='%s' New(blocked)='%s'",
                        concept_id,
                        overlap,
                        HIJACK_BLOCK_THRESHOLD,
                        old_summary[:80],
                        new_data.get("summary", "")[:80],
                    )
                    # Log governance event for telemetry + repeated-blocking failsafe
                    # Note: datetime and UTC already imported at module level (line 23)
                    try:
                        conn.execute(
                            ...
                            (
                                None,
                                "hijack_summary_blocked",
                                concept_id,
                                json.dumps({
                                    "overlap": round(overlap, 4),
                                    "threshold": HIJACK_BLOCK_THRESHOLD,
                                    "old_summary": old_summary[:200],
                                    "blocked_summary": new_data.get("summary", "")[:200],
                                    "version": new_version,
                                }),
                ...
                elif overlap < SUMMARY_OVERLAP_THRESHOLD:
    ...
    clear_stale_risk_metadata(merged_data)
        ...
        conn.execute(
            """UPDATE concepts
               SET version = ?, summary = ?, confidence = ?, concept_type = ?,
                   ...
                   knowledge_area = ?, content_updated_at = ?,
    ...
    try:
        ...
        if FEATURE_FLAGS.get("DRIFT_DETECTION_ENABLED", False):
            from app.cognitive.drift import measure_drift
