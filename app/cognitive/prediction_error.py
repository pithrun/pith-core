...
def assemble_constraint_set(
    ...
    for ac in activated_concepts:
        ...
        try:
            from app.governance.epistemic import apply_epistemic_cap

            authority = apply_epistemic_cap(raw_authority, ac)
        except Exception:
            authority = raw_authority

        # PEC-001 v1.4 Fix 1B: AA authority bypass.
        # always_activate concepts are pinned by explicit user/system intent and
        # must reach constraint_set regardless of assessment-network epistemic
        # cap (0.40 unverified / 0.55 verified < 0.55 threshold). Floor their
        # effective authority at CONSTRAINT_AUTHORITY_THRESHOLD so the gate
        # passes, while preserving the type gate and MAX_CONSTRAINTS truncation
        # downstream. Non-AA concepts follow the original gate unchanged.
        is_aa = bool(ac.get("always_activate", False))
        if is_aa or authority >= CONSTRAINT_AUTHORITY_THRESHOLD:
            ac["_effective_authority"] = max(authority, CONSTRAINT_AUTHORITY_THRESHOLD) if is_aa else authority
...
def validate_previous_response(
    ...
    from app.core.config import CCL_RESPONSE_CAP_CHARS
...
def apply_correction_compound(violations: list[dict]) -> list[dict]:
    ...
    """
    from app.core.config import CCL_MAX_VIOLATIONS_PER_TURN
    ...
    for v in violations:
        ...
        from app.core.config import CCL_AUTHORITY_BOOST_PER_VIOLATION
