...
from app.models import Concept, ConceptEvolution, ConceptProposal, Hypothesis
...
from app.taxonomy import classify_knowledge_area  # KA-001/DEBT-108 (DEBT-112: removed unused imports)
...
def create_concept(proposal: ConceptProposal) -> Concept:
    ...
    try:
        from app.metrics import metrics as _fp_metrics
    ...
    _recompute_governance_scores(concept.id)

    # §3.3: Write-time supersession check for ALL concept types (TB-1: gate removed)
    # BENCH-EVOLVE-001: Skip when PITH_DISABLE_EVOLVE is set (benchmark ingestion).
    import os as _ss_os
    _supersession_disabled = _ss_os.environ.get("PITH_DISABLE_EVOLVE", "").lower() in ("true", "1")
    if _supersession_disabled:
        logger.info("BENCH-EVOLVE-001: Supersession disabled (PITH_DISABLE_EVOLVE=true), skipping for %s", concept.id)
    else:
        try:
            ...
            with db_immediate() as _ss_conn:
                _ss_result = check_supersession_on_write(concept.id, _ss_conn)
                if _ss_result:
                    logger.info(
                        "§3.3: Write-time supersession: %s superseded %s (%s)",
                        concept.id,
            ...
            logger.warning(
                "§3.3: Supersession check failed for %s (non-fatal): %s",
                concept.id,
                _ss_err,
            )
    ...
    try:
        from app.association import auto_associate_decision_concept
...
def _evolve_concept_atomic(evolution: ConceptEvolution) -> Concept | None:
    ...
    # Governance recompute outside transaction (non-fatal, separate conn)
    _recompute_governance_scores(new_concept.id)
    return new_concept
...
def evolve_concept(evolution: ConceptEvolution) -> Concept | None:
    ...
    if FEATURE_FLAGS.get("NONLOSSY_EVOLUTION_ENABLED", False):
        ...
        from app.nonlossy import evolve_concept_nonlossy
        ...
        if result_id:
            ...
            if evolved:
                _recompute_governance_scores(evolved.id)
...
def _maybe_trigger_cascade(evolution: ConceptEvolution, evolved_concept) -> None:
    ...
    from app.config import BENCHMARK as _bm_cascade
    if _bm_cascade.skip_cascades:
        return
    # --- Negative cascade (existing WS3-2) ---
    try:
        from app.config import CASCADE_CORRECTION_THRESHOLD, FEATURE_FLAGS
        ...
        if (
            ...
            from app.cascade import propagate_correction
    ...
    try:
        from app.config import REINFORCEMENT_ENABLED, REINFORCEMENT_EVIDENCE_THRESHOLD
        ...
        from app.cascade import propagate_reinforcement
