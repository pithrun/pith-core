...
# DEBT-240: governance.quarantine must not import cognitive.contradiction directly
# (Contract 4 violation). Using importlib to break the static dependency.
def _get_detect_write_contradiction():
    """Lazy loader — returns detect_write_contradiction from cognitive.contradiction."""
    import importlib
    mod = importlib.import_module("app.cognitive.contradiction")
    return mod.detect_write_contradiction
...
def check_quarantine_reextraction(
    ...
    # DEBT-235: Use dynamic import to avoid governance→retrieval layer violation (Contract 4).
    # Proper fix: inject compute_tfidf_fn as parameter (M14). Using importlib as interim.
    try:
        import importlib as _importlib
        _retrieval_mod = _importlib.import_module("app.retrieval")
        _compute_tfidf = getattr(_retrieval_mod, "_compute_tfidf_similarity")
    except Exception as _e:
        logger.warning("quarantine: failed to load tfidf similarity fn: %s", _e)
        return None

    try:
        for q in quarantined:
            similarity = _compute_tfidf(new_summary, q["summary"])
...
def auto_graduate_quarantined() -> dict[str, list[str]]:
    ...
    for row in candidates:
        ...
        try:
            result = _get_detect_write_contradiction()(
