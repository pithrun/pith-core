...
def execute_supersession(
    ...
    # A5: Escalate status (only if SUPERSEDED > current)
    final_status = escalate_status(old_status, "SUPERSEDED")
    ...
    # KA-006: Sync confidence and currency_status to BOTH column AND blob
    # INGEST-016: Conditionally set valid_until for factual concepts
    _valid_until_sql = ", valid_until = ?" if _old_is_factual else ""
    _valid_until_params = [now] if _old_is_factual else []
    conn.execute(
        f"""UPDATE concepts
           SET currency_status = ?,
               superseded_by = ?,
    ...
    # RETRIEVAL-042 upgrade: Remove superseded concept from FTS5 index
    try:
        from app.storage import _sync_fts5
        _sync_fts5(conn, old_concept_id, delete=True)
    except Exception as _fts_err:
        logger.warning(f"FTS5 delete failed for superseded {old_concept_id}: {_fts_err}")
    ...
    for _eviction_attempt in range(2):  # Max 2 attempts (original + 1 retry)
        try:
            retrieval_engine.remove_concept(old_concept_id)
    ...
    if _eviction_success:
        try:
            # A1: Use concept_id_to_idx dict (not get_document which doesn't exist)
            _still_in_index = old_concept_id in retrieval_engine.index.concept_id_to_idx
            if _still_in_index:
                logger.error(
...
def maybe_create_supersession_cko(
    ...
    try:
        ...
        from app.cko import ensure_cko_table
...
def check_supersession_on_write(new_concept_id: str, conn) -> SupersessionResult | None:
