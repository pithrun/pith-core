...
DEFAULT_PHASES = [1, 2, 3, 4, 5, 6, 7]  # ARCH-D05: phase 6 = promotion, SKILL-DEPLOY-001: phase 7 = synthesis
...
def _write_heartbeat(status: str, details: dict | None = None) -> None:
    ...
    try:
        ...
        if heartbeat_path.exists():
            try:
                existing = json.loads(heartbeat_path.read_text())
            except Exception:
                existing = {}
        heartbeat = {
            "source": "built_in",
        ...
        if existing.get("last_run") and "last_run" not in heartbeat:
