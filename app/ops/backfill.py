...
from app.core.config import EVOLUTION_COSINE_MAX, EVOLUTION_COSINE_MIN
...
from app.cognitive.supersession import TYPE_RANK, execute_supersession
...
def generate_candidates(knowledge_area: str, window_days: int = DEFAULT_WINDOW_DAYS):
    ...
    """
    from app.storage.embedding import embedding_engine as emb
...
def rollback_batch(batch_id: str) -> int:
    ...
    with _db_immediate() as conn:
        ...
        for row_id, older_id, newer_id in committed:
            ...
            if changed:
                conn.execute(
                    "UPDATE concepts SET confidence = min(1.0, confidence + 0.3) WHERE id = ?",
                    (older_id,),
                )
            conn.execute(
