...
def _load_taxonomy():
    ...
    """
    global _canonical_areas, _alias_map, _loaded
    if _loaded:
        return
        ...
        for area in seed_areas:
            db.execute(
                """INSERT OR IGNORE INTO knowledge_areas (name, status, source, description)
                   VALUES (?, 'seed', 'seed', ?)""",
                (area, _CANONICAL_KA_DESCRIPTIONS.get(area, ""))
            )
        db.commit()
    ...
    # DEBT-109: Validate _CANONICAL_KA_DESCRIPTIONS keys match seed data
    desc_keys = set(_CANONICAL_KA_DESCRIPTIONS.keys())
...
def _ensure_provisional(name: str, source: str = "novel"):
    ...
    KA-ARCH-001 Fix 4.
    """
    from app.config import get_feature_flag, KA_PROVISIONAL_MAX
...
def _ensure_canonical_ka_embeddings():
    ...
    try:
        ...
        _canonical_ka_keys = list(_CANONICAL_KA_DESCRIPTIONS.keys())
        texts = list(_CANONICAL_KA_DESCRIPTIONS.values())
...
def _recount_knowledge_areas(db):
    ...
    """
    db.execute("""
        UPDATE knowledge_areas SET
            concept_count = (
                SELECT COUNT(*) FROM concepts
                WHERE knowledge_area = knowledge_areas.name AND is_current = 1
            ),
            last_seen = COALESCE(
                (SELECT MAX(created_at) FROM concepts
                 WHERE knowledge_area = knowledge_areas.name AND is_current = 1),
                knowledge_areas.last_seen
            ),
...
def promote_knowledge_areas() -> dict[str, str]:
    ...
    # First: recount all concept_counts from ground truth
    _recount_knowledge_areas(db)

    # --- Promotion: provisional → established ---
    candidates = db.execute(
        """SELECT ka.name, ka.concept_count
           FROM knowledge_areas ka
           WHERE ka.status = 'provisional'
             AND ka.concept_count >= ?""",
        (KA_PROMOTION_MIN_CONCEPTS,)
    ).fetchall()

    for name, count in candidates:
        session_count = db.execute(
            """SELECT COUNT(DISTINCT session_id)
               FROM concepts WHERE knowledge_area = ? AND is_current = 1""",
            (name,)
        ).fetchone()[0]

        if session_count >= KA_PROMOTION_MIN_SESSIONS:
            db.execute(
                "UPDATE knowledge_areas SET status = 'established', updated_at = datetime('now') WHERE name = ?",
                (name,)
            )
            transitions[name] = "provisional → established"
            _canonical_areas.add(name)
            logger.info(f"KA promoted: '{name}' → established (concepts={count}, sessions={session_count})")
            _generate_ka_description(db, name)

    # --- Promotion: established → mature ---
    mature_candidates = db.execute(
        """SELECT name, concept_count FROM knowledge_areas
           WHERE status = 'established'
             AND concept_count >= ?
             AND julianday('now') - julianday(first_seen) >= ?""",
        (KA_MATURITY_MIN_CONCEPTS, KA_MATURITY_MIN_AGE_DAYS)
    ).fetchall()

    for name, count in mature_candidates:
        db.execute(
            "UPDATE knowledge_areas SET status = 'mature', updated_at = datetime('now') WHERE name = ?",
            (name,)
        )
        transitions[name] = "established → mature"
        logger.info(f"KA matured: '{name}' → mature (concepts={count})")

    # --- Decay: inactive → archived (skip seed KAs) ---
    stale = db.execute(
        """SELECT name, status FROM knowledge_areas
           WHERE status NOT IN ('seed', 'archived')
             AND julianday('now') - julianday(last_seen) >= ?""",
        (KA_DECAY_INACTIVE_DAYS,)
    ).fetchall()

    for name, old_status in stale:
        db.execute(
            "UPDATE knowledge_areas SET status = 'archived', updated_at = datetime('now') WHERE name = ?",
            (name,)
        )
        transitions[name] = f"{old_status} → archived"
        _canonical_areas.discard(name)
        logger.info(f"KA archived: '{name}' (inactive {KA_DECAY_INACTIVE_DAYS}d)")
...
def _record_promotion_run() -> None:
    ...
    set_metadata("last_ka_promotion_at", _utc_now_iso())
