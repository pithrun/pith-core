...
FIRMWARE_CATALOG = [
    # -- Tool routing --
    (
        "desktop_commander_for_host_paths",
        "When accessing files on the user's host machine, "
        "use Desktop Commander tools (read_file, write_file, execute_command) "
        "instead of bash if running inside a sandboxed environment. "
        "Verify file access paths match the actual runtime context.",
        "tool_routing",
    ),
    # -- Protocol essentials --
    (
    ...
    (
        ...
        "Save a pith_checkpoint every 15 minutes of substantive work to prevent "
        "knowledge loss from session drops. Include: description, active item, "
        "done list, next items, and any blockers.",
        "protocol",
    ),
]
...
def seed_firmware() -> dict:
    ...
    if current_version == FIRMWARE_VERSION:
        logger.info(
            f"seed_firmware: firmware v{FIRMWARE_VERSION} already seeded, skipping ({len(FIRMWARE_CATALOG)} entries)"
        )
        return {
            "action": "skipped",
    ...
    logger.info(f"seed_firmware: seeded {seeded}/{len(FIRMWARE_CATALOG)} entries")
