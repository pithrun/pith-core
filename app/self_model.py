...
class SelfModelManager:
    ...
    def get_blind_spots(self) -> list[BlindSpot]:
        ...
        try:
            model = self.load()
            if model and model.epistemic_profile and model.epistemic_profile.knowledge_health:
                return model.epistemic_profile.knowledge_health.blind_spots or []
    ...
    def _compute_calibration(self, concepts: list[Concept]) -> ConfidenceCalibration:
        ...
        """
        try:
            from app.config import CALIBRATION_MIN_PREDICTIONS_FOR_ECE, CALIBRATION_UNTESTED_RATIO_THRESHOLD
            from app.traces import get_calibration_data
        except ImportError:
        ...
        try:
            from app.provenance import get_test_status_label
    ...
    def _compute_blind_spots(self, concepts: list[Concept], areas: dict[str, list[Concept]]) -> list[BlindSpot]:
        ...
        """
        try:
            from app.config import BLIND_SPOT_MIN_CONCEPTS
        except ImportError:
