...
def _concept_to_scored(row: dict):
    ...
    try:
        from app.contradiction import ScoredConcept
...
def detect_write_conflict(
    ...
    try:
        ...
        from app.contradiction import ContradictionType, _phase_1_check
        from app.storage import _db
        ...
        for cand_dict in candidates:
            try:
                ...
                # Phase 1: keyword negation check
                result = _phase_1_check(new_scored, cand_scored)

                # A4: Cross-session — force deeper check if Phase 1 misses
                if result is None:
                    # Bypass _phase_2_check's internal CROSS_TOPIC_OVERLAP_MIN gate
                    # by checking embedding similarity + directional opposition directly
                    if new_scored.embedding is not None and cand_scored.embedding is not None:
                        try:
                            ...
                            # If internal functions aren't importable, skip deep check
                            pass
