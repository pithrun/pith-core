...
Usage:
    python -m app.bridge --source default --target work
    python -m app.bridge --source default --target work --dry-run
    python -m app.bridge --source default --target work --backfill-only
    python -m app.bridge --source default --target work --analyze
"""
...
from app.governance.policy_engine import validate_concept
...
class BridgeConfig:
    ...
    # Trust filters — calibrated against production profile data (avg conf 0.266)
...
def analyze_source(profile: str) -> dict:
    ...
    Usage: python -m app.bridge --source default --analyze
    """
