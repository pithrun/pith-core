...
from app.core.config import EXPERIMENT_VALID_TYPES, experiment_config
...
def generate_synthesis_candidates(
    ...
    # PERF-030: Stratified corpus cap — same pattern as PERF-025 analogy cap.
    # 5/KA by salience bounds O(N²) pair enumeration to ~8K pairs (from 20.5M).
    per_ka = cfg.get("max_pairwise_per_ka", 5)
    try:
        env_override = int(os.environ.get("PITH_EXP_SYNTHESIS_CAP", "0"))
    except (ValueError, TypeError):
        env_override = 0
    if env_override > 0:
        per_ka = max(env_override, 2)  # Floor: need ≥2 concepts for cross-KA pairs
    if per_ka > 0 and len(filtered) > per_ka:
        ...
        if len(filtered) < original_len:
            logger.info(
                "generate_synthesis_candidates: stratified corpus cap %d→%d (%d KAs, %d/KA)",
                original_len,
                len(filtered),
                len(ka_buckets),
                per_ka,
...
def generate_hypothesis_candidates(
    ...
    # PERF-030: Stratified corpus cap — hypothesis needs HIGHER cap than synthesis
    # because _greedy_cluster requires density for min_cluster_size=3 clusters.
    # Empirical: per_ka=5 → 0 clusters, per_ka=10 → 4, per_ka=20 → 10 clusters.
    # NOTE: max_pairwise_per_ka must produce enough concepts for _greedy_cluster
    # to form clusters of min_cluster_size. If cluster_threshold is lowered,
    # this cap may need to increase. See PHASE3_TIMEOUT_RCA_2026_04_13.
    per_ka = cfg.get("max_pairwise_per_ka", 20)
    try:
        env_override = int(os.environ.get("PITH_EXP_HYPOTHESIS_CAP", "0"))
    except (ValueError, TypeError):
        env_override = 0
    if env_override > 0:
        per_ka = max(env_override, 10)  # Floor: need ≥10/KA for meaningful clusters
    if per_ka > 0 and len(filtered) > per_ka:
        ka_buckets: dict[str, list] = defaultdict(list)
        for c in sorted(filtered, key=lambda c: getattr(c, "salience", 0.5) or 0.5, reverse=True):
            ka = getattr(c, "knowledge_area", "general")
        ...
        if len(filtered) < original_len:
            logger.info(
                "generate_hypothesis_candidates: stratified corpus cap %d→%d (%d KAs, %d/KA)",
                original_len,
                len(filtered),
                len(ka_buckets),
                per_ka,
...
def generate_counterfactual_candidates(
    ...
    # PERF-030: Cap seeds by confidence. The bottleneck is seed_count × graph_walk,
    # not adjacency build (59ms) or per-walk cost. 200 seeds → 36ms walk total.
    # Confidence-based selection is semantically appropriate: counterfactual asks
    # "what if this high-confidence thing weren't true?" — low-confidence seeds
    # are less interesting counterfactual targets.
    max_seeds = cfg.get("max_seeds", 200)
    try:
        env_override = int(os.environ.get("PITH_EXP_CF_MAX_SEEDS", "0"))
    except (ValueError, TypeError):
        env_override = 0
    if env_override > 0:
        max_seeds = max(env_override, 50)  # Floor: need ≥50 for candidate diversity
    if len(seeds) > max_seeds:
        original_seed_count = len(seeds)
        seeds = sorted(seeds, key=lambda c: c.confidence, reverse=True)[:max_seeds]
        logger.info(
            "generate_counterfactual_candidates: seed cap %d→%d (top by confidence)",
            original_seed_count,
            max_seeds,
        )
...
def _load_embeddings_for_ids(concept_ids: list[str]) -> dict[str, "np.ndarray"]:
    ...
    from app.storage.embedding import EMBEDDING_DIM, EMBEDDING_VERSION
...
def detect_demand_side_analogies(
    ...
    from app.core.config import experiment_config
    from app.storage.embedding import embedding_engine
...
def process_experiment_results(experiment_id: str, result: ExperimentResult) -> dict:
    ...
    for spec in result.concepts_produced:
        ...
        try:
            from app.cognitive.learning import create_concept
            from app.core.models import ConceptProposal
...
def generate_experiment(
    ...
    # PERF-030: Quality metrics for cap impact monitoring.
    # Computed here (not in generators) to avoid changing generator return signatures.
    corpus_size = len(concepts)
    type_cfg = config_snapshot.get(experiment_type.replace("_detection", "").replace("_generation", "").replace("cross_domain_", ""), {})
    cap_value = type_cfg.get("max_pairwise_per_ka", type_cfg.get("max_seeds", 0))
    quality_metrics = {
        "corpus_size": corpus_size,
        "candidates_generated": len(candidates),
        "cap_active": cap_value > 0 and corpus_size > cap_value,
        "generation_time_ms": gen_time,
    }
    # Per-type enrichment from candidate score_components
    if candidates and experiment_type == "cross_domain_synthesis":
        ka_pairs = set()
        for c in candidates:
            kp = (c.score_components or {}).get("ka_pair", "")
            if kp:
                ka_pairs.add(kp)
        quality_metrics["ka_pair_diversity"] = len(ka_pairs)
    elif candidates and experiment_type == "hypothesis_generation":
        quality_metrics["cluster_count"] = len(candidates)
