...
import app.retrieval.causal as causal
import app.retrieval.temporal as temporal
...
def infer_dates(message: str, user_timezone_offset: int = 0) -> dict[str, str | None]:
    ...
    # Word-number relative date patterns: "four weeks ago", "three days ago"
    match = re.search(
        r"\b(?P<num>one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+weeks?\s+ago\b",
        text_lower,
    )
    if match:
        num_weeks = _NUMBER_WORDS[match.group("num")]
        since_date = today - timedelta(weeks=num_weeks)
        until_date = today
        return {"since": since_date.isoformat(), "until": until_date.isoformat()}

    match = re.search(
        r"\b(?P<num>one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+days?\s+ago\b",
        text_lower,
    )
    if match:
        num_days = _NUMBER_WORDS[match.group("num")]
        since_date = today - timedelta(days=num_days)
        until_date = today
        return {"since": since_date.isoformat(), "until": until_date.isoformat()}
...
def _dispatch_temporal_activity(
    ...
    try:
        ...
        if since is None and until is None:
            logger.info("S4.5: temporal_activity dispatch skipped (no parsed date window)")
            return []
