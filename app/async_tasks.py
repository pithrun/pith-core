...
Task priorities:
  ...
  P2: Association discovery, staleness alerts, self-model update, index rebuild
...
from app.constants import GOV_EVENT_STALENESS_ALERT
...
def get_degraded_tasks(conn) -> list[dict]:
    ...
    for task_type, config in TASK_CONFIGS.items():
        ...
        if not last_run:
            degraded.append({"task_type": task_type, "reason": "never_run"})
            continue
        try:
...
async def run_edge_reclassification(conn, batch_size: int = 100) -> int:
    ...
    from app.config import EDGE_LLM_BATCH_SIZE, EDGE_LLM_RECLASSIFICATION_ENABLED
...
async def _classify_edge_llm(src: tuple, tgt: tuple) -> tuple[str | None, float]:
    ...
    try:
        ...
        # Strip markdown code block wrapper (Haiku frequently wraps JSON)
...
async def run_ka_reclassification(conn, batch_size: int = 50, full_corpus: bool = False) -> int:
    ...
    from app.taxonomy import classify_ka_by_embedding
    ...
    from app.config import KA_LLM_MAX_PER_RUN, KA_LLM_RECLASSIFICATION_ENABLED
    from app.taxonomy import _CANONICAL_KA_DESCRIPTIONS
...
class AsyncTaskRunner:
    ...
    async def run_task(self, task_type: str, conn, **kwargs) -> dict:
        ...
        run_id = record_task_start(task_type, conn)
        ...
        while True:
            try:
                ...
                items = await self._execute_task(task_type, conn, config, **kwargs)
                elapsed = time.perf_counter() - t0

                record_task_complete(run_id, "success", items, None, conn)
                self._last_run_times[task_type] = time.time()

                logger.info("Async task %s: %s items in %.1fs", task_type, items, elapsed)
                return {"status": "success", "items_processed": items, "elapsed_seconds": round(elapsed, 1)}

            except TimeoutError:
                record_task_complete(run_id, "timeout", 0, "Exceeded max runtime", conn)
                return {"status": "timeout", "error": "Exceeded max runtime"}

            except Exception as e:
                retry_count += 1
                if retry_count > max_retries:
                    record_task_complete(run_id, "failed", 0, str(e), conn)
    ...
    async def _execute_task(self, task_type: str, conn, config: TaskConfig, **kwargs) -> int:
        ...
        if task_type == "authority_recalibration":
            ...
            from app.skills import extract_skills_from_corrections
            ...
            from app.self_model import update_self_model
    ...
    async def run_scheduled_tasks(self, conn) -> dict[str, dict]:
        ...
        for task_type, config in sorted(TASK_CONFIGS.items(), key=lambda x: x[1].priority.value):
            if config.interval_hours <= 0:
                continue  # On-demand only
            ...
            result = await self.run_task(task_type, conn)
    ...
    async def run_session_end_tasks(self, session_id: str, conn, gov_ctx=None) -> dict[str, dict]:
        ...
        # Correction-to-skill pipeline
        results["correction_to_skill"] = await self.run_task("correction_to_skill", conn, gov_ctx=gov_ctx)

        # Self-model update
        results["self_model_update"] = await self.run_task("self_model_update", conn)

        # KA-ARCH-001: Dynamic KA lifecycle evolution
        try:
            from app.taxonomy import promote_knowledge_areas, detect_ka_merges
