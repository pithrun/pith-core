...
from app.models import Experiment, ExperimentCandidate, ExperimentResult
...
async def _call_anthropic(prompt: str) -> str:
    """Call Anthropic API with Haiku model. Returns raw text response."""
    global _LLM_AUTH_FAILED
    if _LLM_AUTH_FAILED:
        raise RuntimeError("EXP-018: LLM disabled — ANTHROPIC_API_KEY invalid this run")
    ...
    try:
        ...
        logger.error(
            "EXP-018: LLM disabled — API key rejected (401). "
        ...
        _LLM_AUTH_FAILED = True
        logger.error(
            "EXP-018: LLM disabled — API key lacks permissions (403). "
            "Check key scopes at console.anthropic.com. Error: %s",
            e,
        )
        raise
    except anthropic.BadRequestError as e:
        # [EXP-019] Credit depletion is a permanent account-level block — trip circuit breaker
        if "credit" in str(e).lower() or "billing" in str(e).lower():
            _LLM_AUTH_FAILED = True
            logger.error(
                "EXP-018: LLM disabled — API credits depleted (400). "
...
async def resolve_experiment(
    ...
    """
    from app.config import FEATURE_FLAGS as app_config
