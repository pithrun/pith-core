...
try:
    from app.config import PWT_SOURCE_WEIGHTS, PWT_SUBSTANTIVE_SOURCE_TYPES
except ImportError:
    PWT_SOURCE_WEIGHTS = {
...
def check_corroboration(concept: Concept) -> bool:
    ...
    """
    try:
        from app.config import PWT_CORROBORATION_CONFIDENCE_THRESHOLD
    except ImportError:
...
def check_velocity_anomaly(concept: Concept) -> str | None:
    ...
    """
    try:
        from app.config import PWT_VELOCITY_CLAMP, PWT_VELOCITY_CONFIDENCE_THRESHOLD
    except ImportError:
...
def clamp_velocity_anomaly(concept: Concept) -> float:
    ...
    """
    try:
        from app.config import PWT_VELOCITY_CLAMP
    except ImportError:
        PWT_VELOCITY_CLAMP = 0.6
...
def apply_preference_floor(concept: Concept, computed_salience: float) -> float:
    ...
    """
    try:
        from app.config import PREFERENCE_SALIENCE_FLOOR
    except ImportError:
