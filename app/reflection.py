...
from app.quarantine import auto_graduate_quarantined
...
from app.models import Concept, Evidence, ReflectionSummary
...
RECALIBRATION_DAMPING = 0.15
RECALIBRATION_GAP_THRESHOLD = 0.05  # Dead zone: skip if gap < this (was 0.10 — MEASURE-005)
...
from app.constants import LEGACY_EVIDENCE_STRENGTH, RECENCY_FLOOR, RECENCY_LAMBDA
...
class ReflectionEngine:
    ...
    def reflect(self, mode: str = "incremental") -> ReflectionSummary:
    ...
    def _incremental_reflection(self) -> ReflectionSummary:
        ...
        _s = time.monotonic()
        recalibrated, factor_cvs = self._recalibrate_confidence()
    ...
    def _associate_recently_changed(self, candidate_ids: list[str]) -> int:
        ...
        from app.config import ASSOC_REFLECTION_MAX_PER_CYCLE
    ...
    def _full_reflection(self) -> ReflectionSummary:
        ...
        _s = time.monotonic()
        try:
            from app.rename_registry import scan_for_drift
        ...
        _s = time.monotonic()
        strengthened, strengthened_ids, stability_guarded = self._strengthen_accessed()
        ...
        _s = time.monotonic()
        time_matured, matured_ids = self._time_based_maturation()
        _t["time_maturation"] = _elapsed_ms(_s)

        _s = time.monotonic()
        assoc_propagated = self._propagate_association_confidence()
        ...
        self_model_manager.generate(all_concepts)
    ...
    def _strengthen_accessed(self) -> tuple[int, list[str], int]:
    ...
    def _time_based_maturation(self) -> tuple[int, list[str]]:
    ...
    def _propagate_association_confidence(self) -> int:
    ...
    def _merge_duplicates(self) -> int:
        ...
        for concept_id in all_ids:
            ...
            for match_id, similarity in matches:
                ...
                # Record merge traceability via direct SQL
                # (save_concept does not persist superseded_by; archive_concept
                #  deletes association edges — so we use lightweight direct SQL)
                try:
                    with _db() as conn:
                        ...
                        conn.execute(
                            """UPDATE concepts
                               ...
                               WHERE id = ?""",
                            (survivor.id, _decayed_conf, survivor.id, _decayed_conf, loser.id),
                        )
                except Exception as e:
                ...
                archive_concept(loser.id)  # Fix 3: archive_concept now auto-removes from index
                merged += 1
    ...
    def _recalibrate_confidence(self) -> tuple[int, dict[str, float | None]]:
        ...
        Rules:
          - If confidence > E(c): decay toward E(c) with RECALIBRATION_DAMPING
          - If confidence < E(c): boost toward E(c) with same damping
    ...
    def _run_graduation_sweep(self, _t: dict) -> tuple[int, int]:
        ...
        try:
            _s = time.monotonic()
            grad_result = auto_graduate_quarantined()
    ...
    def _quarantine_release_sweep(self) -> int:
        ...
        """
        from app.config import FEATURE_FLAGS, QUARANTINE_RELEASE_AGE_DAYS, QUARANTINE_RELEASE_CAP
    ...
    def analyze_stability(self) -> dict:
        ...
        from app.config import HEALTH_FRESHNESS_HALF_LIFE_DAYS
